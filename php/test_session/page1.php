...
//https://www.php.net/manual/ru/session.configuration.php#ini.session.gc-divisor
 session.gc_probability integer
	...
	определ€ет веро€тность запуска функции сборщика мусора 
	(gc, garbage collection). ѕо умолчанию равен 1. 
	—м. подробнее в session.gc_divisor. 
session.gc_divisor integer
    session.gc_divisor в сочетании с session.gc_probability 
	определ€ет веро€тность запуска функции сборщика мусора 
	(gc, garbage collection) при каждой инициализации сессии. 
	¬еро€тность рассчитываетс€ как gc_probability/gc_divisor, 
	то есть 1/100 означает, что функци€ gc запускаетс€ в одном случае из ста, 
...
session.gc_maxlifetime integer
	...
	после которой данные будут рассматриватьс€ как "мусор" и 
	потенциально будут удалены. —бор мусора может произойти 
	в течение старта сессии 
	(в зависимости от значений session.gc_probability и session.gc_divisor).
    «амечание: ≈сли разные скрипты имеют разные значени€ session.gc_maxlifetime, но при этом одни и те же места дл€ хранени€ данных сессии, то скрипт с минимальным значением уничтожит все данные. ¬ таком случае следует использовать эту директиву вместе с session.save_path. 
