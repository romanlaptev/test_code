...
remove_action( 'wp_head', 'catalyst_build_custom_css', 15 );


add_action('wp_head','add_seo_meta');
function add_seo_meta(){
    global $meta_description, $meta_keywords , $yandex_verification, $wmail_verification;
    $yandex_verification = '4f8e99e21221d72c';
    $google_verification = '7ml_mS9pkssZbakEiqK4ZpP4jVgaFcdnFMQYp8dhcwQ';
    echo ( $meta_description ? '<meta name="description" content="' . $meta_description . '">' . "\n" : '' );
    echo ( $meta_keywords ? '<meta name="keywords" content="' . $meta_keywords . '" />' . "\n" : '' );
    echo ( $yandex_verification ? '<meta name="yandex-verification" content="' . $yandex_verification . '" />' . "\n" : '' );
    echo ( $wmail_verification ? '<meta name="wmail-verification" content="' . $wmail_verification . '" />' . "\n" : '' );
    echo ( $wmail_verification ? '<meta name="wmail-verification" content="' . $wmail_verification . '" />' . "\n" : '' );
    echo ( $google_verification ? '<meta name="google-site-verification" content="' . $google_verification . '" />' . "\n" : '' );
    //echo '<link rel="shortcut icon" href="' . CHILD_THEME_URL_BLOG . '/favicon.png" />' . "\n";
    echo '<meta name="mailru-domain" content="pnmvI0j8lSpOAkhs" />' . "\n";
...
function load_my_styles(){
	...
	if( !is_admin() ){
		...
		wp_enqueue_style('tooltipster', '/wp-content/uploads/jquery-plugins/tooltipster-master/tooltipster.css');
		wp_enqueue_style('tooltipster-light', '/wp-content/uploads/jquery-plugins/tooltipster-master/themes/tooltipster-light.css');
	}
...
function load_my_scripts4(){
	...
	if ( !is_admin() ){
		...
		wp_register_script('mootools', "/wp-content/uploads/jscart/mootools.js", false);
		wp_enqueue_script('mootools');
	
		wp_register_script('kiCookies', "/wp-content/uploads/jscart/ki.Cookies.js", false);
		wp_enqueue_script('kiCookies');
	
		wp_register_script('kiJSCart', "/wp-content/uploads/jscart/ki.JSCart.js", false);
		...
		wp_register_script('tooltipster', '/wp-content/uploads/jquery-plugins/tooltipster-master/jquery.tooltipster.min.js', false);
		wp_enqueue_script('tooltipster');
		
		wp_register_script('theme_script', '/wp-content/themes/agropit_theme/scripts/theme_script.js', false);
	...
	if ( is_front_page() ){
		...
		wp_register_script('slimbox-jquery', "/wp-content/uploads/slimbox2/slimbox2.js", false);
	...
	if ( !is_admin() && !is_singular() ){
		...
		if( !is_wp_error( $thisCat ) ){
			...
			foreach($categories as $category) {
				$agropit_cat .= '<li><a href="' . get_category_link( $category->term_id ) . '" title="' . sprintf( __( "View all posts in %s" ), $category->name ) . '" ' . '>' . $category->name . '</a> <span class="amount-item">Р”РѕРєСѓРјРµРЅС‚РѕРІ ' . $category->count . '</span></li>' . "\n";
			}
...
function agropit_post_title_action()
{
	$num_tu = get_post_meta( get_the_ID(), 'agropit_ntu_post', 1 );
        ...
        //var_dump( strpos( $num_tu, 'РЎРўРћ' ) === false ); var_dump( strpos( $num_tu, 'РўРўРљ' ) === false );
        
        if( strpos( $num_tu, 'РЎРўРћ' ) !== false || strpos( $num_tu, 'РўРўРљ' ) !== false ) {
            $str_num_tu = $num_tu;	
        }elseif( $num_tu != "" ) {
            $str_num_tu = 'РўРЈ ' . $num_tu;	
        }else{
            $str_num_tu = $num_tu;
           }
	...
	elseif( is_singular() )
	{
		//$post_title = '<h1 class="entry-title">' . $str_num_tu . ' В«' . get_the_title() . 'В»</h1>' . "\n";
	...
	{
		//$post_title = '<h2 class="entry-title"><a href="' . get_permalink() . '" rel="bookmark">' . $str_num_tu. ' В«' . get_the_title() . 'В»</a></h2>' . "\n";
...
function agropit_admin_post_title( $title ){
	...
	if( is_admin() ){
		...
		if ( get_post_meta( get_the_ID(), 'agropit_ntu_post', 1 ) != "" ) {
			...
			$str_num_tu = 'РўРЈ ' . get_post_meta( get_the_ID(), 'agropit_ntu_post', 1 ) . ' ';
...
function agropit_show_news_blog(){
	...
	if ( is_front_page() ){
		...
		?><div id="content"><header class="entry-header"><h1 class="entry-title">РќРѕРІРѕСЃС‚Рё</h1></header></div><?php 
...
function my_extra_fields() {
	...
	if( true ){
		add_meta_box( 'extra_fields', 'РџР°СЂР°РјРµС‚СЂС‹ РґРѕРєСѓРјРµРЅС‚Р°', 'extra_fields_box_func', 'post', 'normal', 'high' );
	}
}
...
function extra_fields_box_func( $post ){
	...
	?>
  <p><label>Р¦РµРЅР° - <input type="text" name="agropit_price_post" value="<?php echo get_post_meta( $post->ID, 'agropit_price_post', 1 ); ?>" style="width:50%" /> СЂСѓР±.</label></p> 
 	<p><label>в„– РўРЈ - <input type="text" name="agropit_ntu_post" value="<?php echo get_post_meta( $post->ID, 'agropit_ntu_post', 1 ); ?>" style="width:50%" /></label></p>
 	<p><label>РўРѕР»СЊРєРѕ РґР»СЏ kupi-tu - <input type="checkbox" name="agropit_only_kupitu" <?php if(get_post_meta( $post->ID, 'agropit_only_kupitu', 1 )=='on'){echo 'checked="checked"';} ?> ></label></p>
...
/* РЎРѕС…СЂР°РЅСЏРµРј РґР°РЅРЅС‹Рµ, РїСЂРё СЃРѕС…СЂР°РЅРµРЅРёРё РїРѕСЃС‚Р° */
function my_extra_fields_update( $post_id ){
	if ( !wp_verify_nonce($_POST['extra_fields_nonce'], __FILE__) ) return false; // РїСЂРѕРІРµСЂРєР°
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return false; // РІС‹С…РѕРґРёРј РµСЃР»Рё СЌС‚Рѕ Р°РІС‚РѕСЃРѕС…СЂР°РЅРµРЅРёРµ
	if ( !current_user_can( 'edit_post', $post_id ) ) return false; // РІС‹С…РѕРґРёРј РµСЃР»Рё СЋР·РµСЂ РЅРµ РёРјРµРµС‚ РїСЂР°РІРѕ СЂРµРґР°РєС‚РёСЂРѕРІР°С‚СЊ Р·Р°РїРёСЃСЊ

	if( !isset( $_POST['agropit_price_post'] ) && !isset( $_POST['agropit_ntu_post'] ) ) return false; // РІС‹С…РѕРґРёРј РµСЃР»Рё РґР°РЅРЅС‹С… РЅРµС‚

	// Р’СЃРµ РћРљ! РўРµРїРµСЂСЊ, РЅСѓР¶РЅРѕ СЃРѕС…СЂР°РЅРёС‚СЊ/СѓРґР°Р»РёС‚СЊ РґР°РЅРЅС‹Рµ
	//$_POST['extra'] = array_map('trim', $_POST['extra']); // РґР»СЏ РјР°СЃСЃРёРІР° С‡РёСЃС‚РёРј РІСЃРµ РґР°РЅРЅС‹Рµ РѕС‚ РїСЂРѕР±РµР»РѕРІ РїРѕ РєСЂР°СЏРј
	//$_POST['agropit_price_post'] = trim( $_POST['agropit_price_post'] ); // С‡РёСЃС‚РёРј РІСЃРµ РґР°РЅРЅС‹Рµ РѕС‚ РїСЂРѕР±РµР»РѕРІ РїРѕ РєСЂР°СЏРј
	if( empty( $_POST['agropit_price_post'] ) ){
		delete_post_meta( $post_id, 'agropit_price_post' ); // СѓРґР°Р»СЏРµРј РїРѕР»Рµ РµСЃР»Рё Р·РЅР°С‡РµРЅРёРµ РїСѓСЃС‚РѕРµ
	}
	else{
		update_post_meta( $post_id, 'agropit_price_post', $_POST['agropit_price_post'] ); // add_post_meta() СЂР°Р±РѕС‚Р°РµС‚ Р°РІС‚РѕРјР°С‚РёС‡РµСЃРєРё
	}
...
function agropit_add_cart_in_post(){
	...
	if( is_single() ){
		...
		<div class="btn-cart-block">	
			...
			?>	
				<div id="btn-add-tu">
				РЎС‚РѕРёРјРѕСЃС‚СЊ:<div class="amount"><?php echo $agropit_post_price ?> СЂСѓР±.</div>
				<span style="color:#eee;font-size:10px;">РќР”РЎ РЅРµ РѕР±Р»Р°РіР°РµС‚СЃСЏ</span>
				<a href="#" class="btn-add" title="Р”РѕР±Р°РІРёС‚СЊ РўРµС…РЅРёС‡РµСЃРєРѕРµ СѓСЃР»РѕРІРёРµ РІ Р·Р°РєР°Р·" onclick="jQuery('.amountItemsCart').text(jsCartChangeItem('cookiesCart', '<?php echo $agropit_post_id; ?>', 1, <?php echo $agropit_post_price; ?> , 'add')); document.getElementById('btn-add-tu').innerHTML='РўРЈ СѓСЃРїРµС€РЅРѕ РґРѕР±РѕРІР»РµРЅРѕ РІ Р·Р°РєР°Р·<br><a title=\'РџРµСЂРµР№С‚Рё Рє Р·Р°РєР°Р·Сѓ...\' class=\'btn-view\' href=\'/cart\'>РћС„РѕСЂРјРёС‚СЊ</a>';">РљСѓРїРёС‚СЊ</a>
				</div>
				
			<?php }
			else {
				?>
				Р­С‚Рѕ РўРЈ СѓР¶Рµ РґРѕР±РѕРІР»РµРЅРѕ РІ Р·Р°РєР°Р·
				<br><a title="РџРµСЂРµР№С‚Рё Рє Р·Р°РєР°Р·Сѓ..." class="btn-view" href="/cart">РџРѕСЃРјРѕС‚СЂРµС‚СЊ</a>
				<?php
			}
...
function add_reviews_form(){
	...
	if ( !is_singular() && is_category( 'reviews' ) ){
		echo do_shortcode( '[contact-form-7 id="4499" title="РћСЃС‚Р°РІСЊС‚Рµ СЃРІРѕР№ РѕС‚Р·С‹РІ"]' );
	}
}
...
function ws_filter_page($html){
	...
	foreach($dom->find('#menu-item-5565 a') as $element){
		$element->innertext = 'РљРѕСЂР·РёРЅР° (<span class="amountItemsCart" style="display:inline">0</span>)';
	}
...
<script id="top100Counter" type="text/javascript" src="https://counter.rambler.ru/top100.jcn?2958791"></script>
<noscript>
<a href="http://top100.rambler.ru/navi/2958791/">
<img src="https://counter.rambler.ru/top100.cnt?2958791" alt="Rambler's Top100" border="0" />
</a>
...
var _tmr = _tmr || [];
_tmr.push({id: "2419364", type: "pageView", start: (new Date()).getTime()});
(function (d, w) {
...
<img src="//top-fwz1.mail.ru/counter?id=2419364;js=na" style="border:0;" height="1" width="1" alt="Р РµР№С‚РёРЅРі@Mail.ru" />
</div>
</noscript>
<!-- //Rating@Mail.ru counter -->
<!-- Rating@Mail.ru logo -->
<a href="http://top.mail.ru/jump?from=2419364">
<img src="//top-fwz1.mail.ru/counter?id=2419364;t=272;l=1" 
style="border:0;" height="31" width="38" alt="Р РµР№С‚РёРЅРі@Mail.ru" /></a>
<!-- //Rating@Mail.ru logo -->
	<?php 
}
