...
*/
	foreach ($files as $file)
	{
		...
		$out .= "<a href='../".$dir."/".$file->basename."' target=_blank>view</a>, ";
		$out .= "<a href='".$_SERVER['SCRIPT_NAME']."?action=delete&filename=".$file->basename."'>delete</a> ";
...
//---------------------------------------------------------------------
	...
	$num_pages=0;
	//foreach ($xml->album as $album)
	foreach ($xml->notebook as $notebook)
	{
		$n1++;
//echo $n1.". ".$notebook[@title];
//echo "<br>";

		$n2=0;
//echo "		<ul>";
		foreach ($notebook->page as $page)
		{
			$n2++;
//echo $n2.". ".$page[@title];
//echo "<br>";
		}
		$num_pages = $num_pages + $n2;
//echo "		</ul>";
	...
	$log .= "тетрадей - ".$num_notebook; 
	$log .= "<br>";
	$log .= "страниц - ".$num_pages; 
	$log .= "<br>";
...
//--------------------------------------------------------
	...
	foreach ($xml->notebook as $notebook)
	{
		foreach ($notebook->page as $node_xml)
		{
			$num_node++;
...
{
	$out = "";
	$out .= ", пропускаем импорт...";
	//$out .= "обновить материал";
	//$out .= "<br>";
...
//--------------------------------
	if (!empty($node_xml->body_value))
	{
		$node->body = $node_xml->body_value;
	}

	//$node->field_content_location[]['value']
	$node->field_large_img[]['value'] = $node_xml->field_large_img_value;
	$node->field_medium_img[]['value'] = $node_xml->field_medium_img_value;
	$node->field_original_img[]['value'] = $node_xml->field_original_img_value;
	$node->field_preview_gallery_img[]['value'] = $node_xml->field_preview_gallery_img_value;
	$node->field_small_img[]['value'] = $node_xml->small_img;
/*
	$files=array();
	$files['fid'] = 20;
	$files['uid'] = 1;
	$files['filename'] = 'p0001.jpg';
	$files['filepath'] = 'sites/default/files/p0001.jpg';
	$files['filemime'] = 'image/jpeg';
	$files['filesize'] = '211778';
	$files['status'] = 1;
	$files['timestamp'] = 1358058328;
	$files['list'] = 1; 
	$files['data'] = array ();
	$files['data']['description'] = "";
	$files['data']['alt'] =""; 
	$files['data']['title'] = "";
	$node->field_upload_img = array(array($files));

	$node->page_title = $node_xml->page_title;
	//$node->path = $node_xml->alias;
	$node->path = "aliassss!";
*/

/*
[nodewords] => Array ( [abstract] => Array ( [value] => ) [canonical] => Array ( [value] => ) 
[copyright] => Array ( [value] => ) 
[description] => Array ( [value] => ) 
[keywords] => Array ( [value] => ) 
[revisit-after] => Array ( [value] => 1 ) 
[robots] => Array ( [value] => Array ( [index] => 0 [noindex] => 0 [follow] => 0 [nofollow] => 0 [noarchive] => 0 [noodp] => 0 [nosnippet] => 0 [noydir] => 0 ) [use_default] => 0 ) ) ) 
...
<body>
<h2>book_history_engraving</h2>
<h3>Импорт XML --> mysql, Drupal 6</h3>
