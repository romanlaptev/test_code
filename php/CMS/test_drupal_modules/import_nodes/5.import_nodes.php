...
//------------------------------------------
				...
				if (!empty($_REQUEST['node_type']))
				{
					...
					if (!empty($_REQUEST['new_vocabulary']))//Создание нового словаря
					{
						...
						drupal_new_vocabulary($node_type,$vocabulary_name);
					}
					
					//$log = drupal_import_from_xml($filename,$node_type);
				}
				else
				{
...
//==========================================================================
		case update_vocabulary:
			...
			{
				$message = "<span class='error'>var vocabulary_vid is empty!!!!</span>";
				$message .= "<br>";
			}
		break;
//==========================================================================
...
//------------------------------------- taxonomy
	$out .= "<div class='section'><b>Словари таксономии.</b> ";
...
//------------------------------------- nodes
	...
	$out .= "<input type=hidden name=action value='import_xml'>";
...
//print_r($row);
//echo "</pre>";
		//$out .= "<input type=radio name=vocabulary_vid value='".$row->vid."'/>".$row->vid."";
		//$out .= "<input type=radio name=vocabulary_name value='".$row->name."'/>".$row->name."<br>";
		$out .= "<input type=radio name='vocabulary_vid' value='".$row->vid."'/>".$row->name."";
		$out .= "<a href='".$_SERVER['SCRIPT_NAME']."?action=update_vocabulary'>импорт</a><br>";
	}
	$out .= "или в новый словарь с названием ";
	$out .= "<input type=text name=new_vocabulary value=''/>";
	//$out .= "<a href='".$_SERVER['SCRIPT_NAME']."?action=new_vocabulary&new_vocabulary=test'>создать</a><br>";
...
//echo "		</ul>";
	...
	$num_albums=$n1;
	$out = "";
	$out .= "В XML-файле найдено: "; 
	$out .= "<ul>";

	$out .= "альбомов - ".$num_albums; 
	$out .= "<br>";
	$out .= "изображений - ".$num_nodes; 
	$out .= "<br>";
...
function drupal_import_from_xml($filename, $node_type)
{
	global $db_prefix;
	$xml = simplexml_load_file($filename);
	if ($xml == FALSE) 
...
`tags`, 
`module`, 
`weight`) VALUES (NULL, '".$new_vocabulary."', NULL, '', '0', '0', '0', '0', '0', '', '0');
";
echo $query;
echo "<br>";
	$res=db_query($query);
//INSERT INTO vocabulary_node_types (`vid`, `type`) VALUES ('5', '".$node_type."');				
} //-------------------------- end func
?>
<html>
<head>
