...
*/
	...
	{
	$out .= "<input type=checkbox name=filename value='".$file->basename."'/>".$file->basename."<br>";
	}
	$out .= "</ul>";
	return $out;
} //-------------------------- end func
...
<input type=submit value='Загрузить'>
	...
	$out .= "<form method=post name=form_import_files action='".$_SERVER['SCRIPT_NAME']."'>
<fieldset>
<legend>Параметры импорта</legend>";

	$out .= "<div class='section'><h2>Список файлов для импорта</h2>";
	...
	$out .= "<div class='section'><h2>Список словарей таксономии</h2>";
	$out .= drupal_view_taxonomy_voc(); //вывод имеющихся словарей таксономии
	$out .= "</div>";
	
	$out .= "<div class='section'><b>action</b><br>";
	//$out .= "<select size=4 name=action onChange='javascript:document.forms.form_import_files.submit();'>";
	$out .= "<select size=4 name=action onChange=''>";
	$out .= "<option value='' selected></option>";
	$out .= "<option value='import_xml'> import_xml</option>";
	$out .= "<option value='import_xml_albums'> import_xml_albums</option>";
	$out .= "<option value='delete'> delete file</option>";
	$out .= "</select>";
	$out .= "</div>";
...
//echo "</pre>";
	...
	$out .= "<input type=text name=vocabulary value=''/>";
	$out .= "<br>";
	return $out;
} //-------------------------- end func
...
function import_from_xml_albums($filename)
{
	global $db_prefix;
	$xml = simplexml_load_file($filename);
	if ($xml == FALSE) 
